...
#include <agipc/agipc_step_statistics.cuh>
                     ...
                     || face.z == _edges[obj_idx].x || face.z == _edges[obj_idx].y))
                {
                    if(!(_btype[face.x] >= 2 && _btype[face.y] >= 2
                                           ...
                                           _vertexes[face.z]))
                        {
                            ...
                            if(atomicCAS(_isIntesect,0,-1)==0 && hit)
                                *hit=make_int2(static_cast<int>(obj_idx),idx);
...
void GIPC::MALLOC_DEVICE_MEM()
{
    ...
    set_collision_pair_capacities(MAX_COLLITION_PAIRS_NUM,MAX_CCD_COLLITION_PAIRS_NUM);
}
...
double GIPC::self_largestFeasibleStepSize(double slackness, double* mqueue, int numbers)
{
    ...
    const unsigned int threadNum = default_threads;
    int                blockNum  = (numbers + threadNum - 1) / threadNum;
    const auto scratch_capacity=std::max(vertexNum,tetrahedraNum);
    if(static_cast<uint32_t>(blockNum)>scratch_capacity) {
        gipc::Statistics::instance().json()["collision_pair_memory"]["ccd_reduction_capacity_failure"]={
            {"required_slots",blockNum},{"allocated_slots",scratch_capacity},{"candidate_pairs",numbers}};
        throw std::runtime_error("CCD reduction scratch capacity exceeded before GPU write");
    }
...
void GIPC::ensure_collision_pair_capacity(uint32_t discrete_required,uint32_t ccd_required)
{
    const auto grow=[](uint32_t required) {
        const std::uint64_t capacity=std::uint64_t(required)+required/4+1;
        if(capacity>std::uint64_t(std::numeric_limits<int>::max()))
            throw std::runtime_error("collision pair capacity exceeds signed index domain");
        return static_cast<int>(capacity);
    };
    const bool discrete_growth=discrete_required>static_cast<uint32_t>(MAX_COLLITION_PAIRS_NUM);
    const bool ccd_growth=ccd_required>static_cast<uint32_t>(MAX_CCD_COLLITION_PAIRS_NUM);
    int4* new_discrete=nullptr; int* new_index=nullptr; int4* new_ccd=nullptr;
    const int discrete_capacity=discrete_growth?grow(discrete_required):MAX_COLLITION_PAIRS_NUM;
    const int ccd_capacity=ccd_growth?grow(ccd_required):MAX_CCD_COLLITION_PAIRS_NUM;
    auto allocate=[&](void** data,std::size_t bytes) {
        const auto error=cudaMalloc(data,bytes);
        if(error!=cudaSuccess)
        {
            cudaFree(new_discrete); cudaFree(new_index); cudaFree(new_ccd);
            std::size_t free_bytes=0,total_bytes=0; cudaMemGetInfo(&free_bytes,&total_bytes);
            auto& memory=gipc::Statistics::instance().json()["collision_pair_memory"];
            memory["allocation_failure"]={{"requested_bytes",bytes},{"cuda_error",cudaGetErrorString(error)},
                {"free_bytes",free_bytes},{"total_bytes",total_bytes}};
            throw std::runtime_error(std::string("collision pair allocation failed: ")+cudaGetErrorString(error));
        }
    };
    if(discrete_growth)
    {
        allocate(reinterpret_cast<void**>(&new_discrete),std::size_t(discrete_capacity)*sizeof(int4));
        allocate(reinterpret_cast<void**>(&new_index),std::size_t(discrete_capacity)*sizeof(int));
    }
    if(ccd_growth) allocate(reinterpret_cast<void**>(&new_ccd),std::size_t(ccd_capacity)*sizeof(int4));
    const std::size_t allocated=std::size_t(discrete_capacity)*(sizeof(int4)+sizeof(int))
        +std::size_t(ccd_capacity)*sizeof(int4);
    const std::size_t transient=allocated
        +(discrete_growth?std::size_t(MAX_COLLITION_PAIRS_NUM)*(sizeof(int4)+sizeof(int)):0)
        +(ccd_growth?std::size_t(MAX_CCD_COLLITION_PAIRS_NUM)*sizeof(int4):0);
    if(discrete_growth)
    {
        CUDA_SAFE_CALL(cudaFree(_collisonPairs)); CUDA_SAFE_CALL(cudaFree(_MatIndex));
        _collisonPairs=new_discrete; _MatIndex=new_index;
        MAX_COLLITION_PAIRS_NUM=discrete_capacity;
    }
    if(ccd_growth)
    {
        CUDA_SAFE_CALL(cudaFree(_ccd_collisonPairs)); _ccd_collisonPairs=new_ccd;
        MAX_CCD_COLLITION_PAIRS_NUM=ccd_capacity;
    }
    bvh_e._collisionPair=bvh_f._collisionPair=_collisonPairs;
    bvh_e._ccd_collisionPair=bvh_f._ccd_collisionPair=_ccd_collisonPairs;
    bvh_e._MatIndex=bvh_f._MatIndex=_MatIndex;
    if(discrete_growth || ccd_growth)
        set_collision_pair_capacities(MAX_COLLITION_PAIRS_NUM,MAX_CCD_COLLITION_PAIRS_NUM);
    auto& memory=gipc::Statistics::instance().json()["collision_pair_memory"];
    memory["discrete_capacity"]=MAX_COLLITION_PAIRS_NUM;
    memory["ccd_capacity"]=MAX_CCD_COLLITION_PAIRS_NUM;
    memory["allocated_bytes"]=allocated;
    memory["required_discrete_pairs_peak"]=std::max(discrete_required,memory.value("required_discrete_pairs_peak",0u));
    memory["required_ccd_pairs_peak"]=std::max(ccd_required,memory.value("required_ccd_pairs_peak",0u));
    memory["allocation_peak_including_growth_bytes"]=std::max(transient,memory.value("allocation_peak_including_growth_bytes",std::size_t(0)));
    if(discrete_growth || ccd_growth)
    {
        if(!memory.contains("growth_events")) memory["growth_events"]=gipc::Json::array();
        memory["growth_events"].push_back({{"discrete_required",discrete_required},{"ccd_required",ccd_required},
            {"discrete_capacity",discrete_capacity},{"ccd_capacity",ccd_capacity},
            {"rebuild_before_use",true}});
    }
}
...
void GIPC::buildCP()
{
    for(;;)
    {
        CUDA_SAFE_CALL(cudaMemset(_cpNum,0,5*sizeof(uint32_t)));
        CUDA_SAFE_CALL(cudaMemset(_gpNum,0,sizeof(uint32_t)));
        bvh_f.SelfCollitionDetect(dHat); bvh_e.SelfCollitionDetect(dHat);
        ...
        const bool retry=h_cpNum[0]>static_cast<uint32_t>(MAX_COLLITION_PAIRS_NUM)
            || h_cpNum[0]>static_cast<uint32_t>(MAX_CCD_COLLITION_PAIRS_NUM);
        ensure_collision_pair_capacity(h_cpNum[0],h_cpNum[0]);
        if(!retry) break;
    }
}
...
void GIPC::buildFullCP(const double& alpha)
{
    for(;;)
    {
        ...
        const bool retry=h_ccd_cpNum>static_cast<uint32_t>(MAX_CCD_COLLITION_PAIRS_NUM);
        ensure_collision_pair_capacity(0,h_ccd_cpNum);
        if(!retry) break;
    }
}
...
int GIPC::calculateMovingDirection(device_TetraData& TetMesh, int cpNum, int preconditioner_type)
{
    ...
    agipc::set_direction_capture_context(total_Frames+1,h_cpNum[0],h_gpNum,_collisonPairs);
                              ...
                              int            number, int2* hit=nullptr)
{
    ...
    _edgeTriIntersectionQuery<<<blockNum, threadNum>>>(
        _btype, _vertexes, _edges, _faces, _edge_bvs, _edge_nodes, _isIntersect, dHat, numbers, hit);
...
namespace
{
struct TrialPositions
{
    const double3* current;
    const double3* previous;
    // bit 0: some position changed; bit 1: a nonfinite position.
    __host__ __device__ int operator()(int i) const
    {
        const auto a=current[i], b=previous[i];
        return ((a.x!=b.x || a.y!=b.y || a.z!=b.z)?1:0)
            | ((!isfinite(a.x)||!isfinite(a.y)||!isfinite(a.z))?2:0);
    }
};
struct CombineTrialFlags
{
    __host__ __device__ int operator()(int a,int b) const { return a|b; }
};
int trial_flags(const double3* current,const double3* previous,int count)
{
    return thrust::transform_reduce(thrust::device,
        thrust::make_counting_iterator(0),thrust::make_counting_iterator(count),
        TrialPositions{current,previous},0,CombineTrialFlags{});
}
template<typename T>
void save_probe(const std::filesystem::path& path,const T* device,std::size_t count)
{
    std::vector<T> data(count);
    if(count) CUDA_SAFE_CALL(cudaMemcpy(data.data(),device,count*sizeof(T),cudaMemcpyDeviceToHost));
    std::ofstream out;
...
bool GIPC::lineSearch(device_TetraData& TetMesh, double& alpha, const double& cfl_alpha)
{
    ...
    const double initial_energy=computeEnergy(TetMesh);
    double tested_energy=initial_energy;
    int energy_backtracks=0, intersection_backtracks=0;
    CUDA_SAFE_CALL(cudaMemcpy(TetMesh.temp_double3Mem,TetMesh.vertexes,
                              vertexNum*sizeof(double3),cudaMemcpyDeviceToDevice));
    m_abd_system->copy_q_to_q_temp(*m_abd_sim_data);
    auto& stats=gipc::Statistics::instance().at_current_frame()["newton"].back()["line_search"];
    if(!stats.is_object()) stats=gipc::Json::object();
    stats.update({{"input_alpha",initial_alpha},{"cfl_alpha",cfl_alpha},
                  {"initial_energy",initial_energy},{"status","pending"}});
    auto fail=[&](const std::string& reason)
    {
        stats["status"]="failed"; stats["failure_reason"]=reason;
        stats["trial_alpha"]=alpha; stats["tested_energy"]=tested_energy;
        stats["accepted_alpha"]=nullptr;
        stats["energy_backtracks"]=energy_backtracks;
        stats["intersection_backtracks"]=intersection_backtracks;
        std::string probe_error;
        const auto directory=std::filesystem::path(gipc::output_dir())/"line_search_failure"
            /std::to_string(std::chrono::system_clock::now().time_since_epoch().count());
        try
        {
            std::filesystem::create_directories(directory);
            save_probe(directory/"trial.f64x3.bin",TetMesh.vertexes,vertexNum);
            save_probe(directory/"initial.f64x3.bin",TetMesh.temp_double3Mem,vertexNum);
            save_probe(directory/"direction.f64x3.bin",_moveDir,vertexNum);
            save_probe(directory/"boundary.i32.bin",TetMesh.BoundaryType,vertexNum);
            save_probe(directory/"edges.u32x2.bin",bvh_e._edges,bvh_e.edge_number);
            save_probe(directory/"faces.u32x3.bin",bvh_f._faces,bvh_f.face_number);
            save_probe(directory/"rest.f64x3.bin",TetMesh.rest_vertexes,vertexNum);
            save_probe(directory/"edge_bvh_nodes.u32x4.bin",bvh_e._nodes,2*bvh_e.edge_number-1);
            save_probe(directory/"edge_bvh_bounds.f64x6.bin",bvh_e._bvs,2*bvh_e.edge_number-1);
            save_probe(directory/"ground_offset.f64.bin",_groundOffset,1);
            save_probe(directory/"ground_normal.f64x3.bin",_groundNormal,1);
            save_probe(directory/"ground_candidate_vertices.u32.bin",_environment_collisionPair,h_gpNum);
            save_probe(directory/"abd_q_initial.f64x12.bin",
                m_abd_sim_data->device.body_id_to_q_temp.data(),abd_fem_count_info.abd_body_num);
            save_probe(directory/"abd_q_trial.f64x12.bin",
                m_abd_sim_data->device.body_id_to_q.data(),abd_fem_count_info.abd_body_num);
            save_probe(directory/"abd_dq.f64x12.bin",
                m_abd_sim_data->device.body_id_to_dq.data(),abd_fem_count_info.abd_body_num);
            cudatool::CudaDeviceBuffer<int2> hit(1);
            CUDA_SAFE_CALL(cudaMemset(hit.data(),0xff,sizeof(int2)));
            if(reason!="nonfinite_trial_positions" && edgeTriIntersectionQuery(bvh_e._btype,TetMesh.vertexes,bvh_e._edges,
                bvh_f._faces,bvh_e._bvs,bvh_e._nodes,dHat,bvh_f.face_number,hit.data()))
            {
                std::vector<int2> ids; hit.copy_to_host(ids);
                stats["intersecting_edge_id"]=ids[0].x;
                stats["intersecting_face_id"]=ids[0].y;
            }
            stats["probe_directory"]=directory.string();
            stats["vertex_count"]=vertexNum;
            stats["edge_count"]=bvh_e.edge_number;
            stats["face_count"]=bvh_f.face_number;
        }
        catch(const std::exception& e) { probe_error=e.what(); }
        // Restore q directly: evaluating q_temp - 0*dq is unsafe for nonfinite dq.
        m_abd_sim_data->device.body_id_to_q=m_abd_sim_data->device.body_id_to_q_temp;
        CUDA_SAFE_CALL(cudaMemcpy(TetMesh.vertexes,TetMesh.temp_double3Mem,
                                  vertexNum*sizeof(double3),cudaMemcpyDeviceToDevice));
        const bool initial_positions_finite=!(trial_flags(TetMesh.vertexes,TetMesh.vertexes,vertexNum)&2);
        stats["restored_initial_positions_finite"]=initial_positions_finite;
        if(initial_positions_finite)
        {
            buildBVH(); buildCP();
            stats["restored_initial_intersected"]=isIntersected(TetMesh);
        }
        else stats["restored_initial_intersected"]=nullptr;
        if(initial_positions_finite && stats["restored_initial_intersected"].get<bool>())
        {
            stats["trial_failure_reason"]=reason;
            stats["failure_reason"]="initial_state_intersected";
        }
        stats["restored_initial_state"]=true;
        if(!probe_error.empty()) stats["probe_error"]=probe_error;
        std::size_t free_bytes=0,total_bytes=0;
        CUDA_SAFE_CALL(cudaMemGetInfo(&free_bytes,&total_bytes));
        stats["device_free_bytes"]=free_bytes; stats["device_total_bytes"]=total_bytes;
        try { std::ofstream out(directory/"failure.json"); out<<stats.dump(2); }
        catch(...) {} // A probe failure must not suppress the solver failure.
        throw std::runtime_error("line search failed: "+reason);
    };
    if(!gipc::valid_line_search_input(alpha,cfl_alpha,initial_energy))
        fail("nonfinite_or_invalid_input");
    cudatool::CudaDeviceBuffer<double3> previous_trial;
    for(int backtracks=0;;++backtracks)
    {
        step_forward(TetMesh,alpha,false);
        const int flags=trial_flags(TetMesh.vertexes,
            previous_trial.size()?previous_trial.data():TetMesh.temp_double3Mem,vertexNum);
        if(flags&2) fail("nonfinite_trial_positions");
        if(backtracks && !(flags&1)) fail("no_representable_position_progress");
        buildBVH();
        // Refresh both contact lists before testing: ground candidates must match this trial.
        buildCP();
        const bool intersected=isIntersected(TetMesh);
        tested_energy=intersected?std::numeric_limits<double>::infinity():computeEnergy(TetMesh);
        if(!intersected && std::isfinite(tested_energy) && tested_energy<=initial_energy)
        {
            stats["status"]="accepted"; stats["accepted_alpha"]=alpha;
            stats["energy_backtracks"]=energy_backtracks;
            stats["intersection_backtracks"]=intersection_backtracks;
            stats["tested_energy"]=tested_energy; stats["tested_energy_nonincreasing"]=true;
            return false;
        }
        if(backtracks>=gipc::line_search_backtrack_limit) fail("backtrack_limit");
        if(intersected) ++intersection_backtracks; else ++energy_backtracks;
        previous_trial.resize(vertexNum);
        CUDA_SAFE_CALL(cudaMemcpy(previous_trial.data(),TetMesh.vertexes,
                                  vertexNum*sizeof(double3),cudaMemcpyDeviceToDevice));
        const double next=gipc::next_line_search_alpha(alpha,cfl_alpha);
        if(!(next>0) || !(next<alpha)) fail("no_representable_alpha_progress");
        alpha=next;
    }
}

gipc::Json agipc::line_search_probe_self_test()
{
    using cudatool::CudaDeviceBuffer;
    auto require=[](bool ok,const char* message) {
        if(!ok) throw std::runtime_error(message);
    };
    const std::vector<double3> initial={make_double3(0,0,0),make_double3(1,0,0),
        make_double3(0,1,0),make_double3(.2,.2,-1),make_double3(.2,.2,1),
        make_double3(2,2,-1),make_double3(2,2,1)};
    CudaDeviceBuffer<double3> positions(initial), before(initial);
    require(trial_flags(positions.data(),before.data(),7)==0,
            "line search stationary state must be detected");
    auto changed=initial; changed[0].x=std::nextafter(0.0,1.0);
    positions.copy_from_host(changed);
    require(trial_flags(positions.data(),before.data(),7)==1,
            "line search must retain representable tiny changes");
    changed[0].x=std::numeric_limits<double>::quiet_NaN();
    positions.copy_from_host(changed);
    require((trial_flags(positions.data(),before.data(),7)&2)!=0,
            "line search must detect nonfinite positions");
    positions.copy_from_host(initial);
    CudaDeviceBuffer<uint2> edges(std::vector<uint2>{make_uint2(3,4),make_uint2(5,6)});
    CudaDeviceBuffer<uint3> faces(std::vector<uint3>{make_uint3(0,1,2)});
    CudaDeviceBuffer<int> boundary(std::vector<int>(7,0));
    std::vector<AABB> boxes(3);
    boxes[1].combines(.2,.2,-1); boxes[1].combines(.2,.2,1);
    boxes[2].combines(2,2,-1); boxes[2].combines(2,2,1);
    boxes[0].combines(boxes[1]); boxes[0].combines(boxes[2]);
    CudaDeviceBuffer<AABB> bvs(boxes);
    CudaDeviceBuffer<Node> nodes(std::vector<Node>{
        {0xffffffff,1,2,0xffffffff},{0,0xffffffff,0xffffffff,0},
        {0,0xffffffff,0xffffffff,1}});
    CudaDeviceBuffer<int2> hit(1);
    require(edgeTriIntersectionQuery(boundary.data(),positions.data(),edges.data(),
        faces.data(),bvs.data(),nodes.data(),1e-6,1,hit.data()),
        "line search primitive probe must find crossing edge");
    std::vector<int2> ids; hit.copy_to_host(ids);
    require(ids[0].x==0 && ids[0].y==0,"line search probe primitive IDs are wrong");
    boundary.copy_from_host(std::vector<int>(7,2));
    require(!edgeTriIntersectionQuery(boundary.data(),positions.data(),edges.data(),
        faces.data(),bvs.data(),nodes.data(),1e-6,1),
        "line search probe must honor fixed geometry filtering");
    return {{"passed",true},{"cases",5},
            {"scope","production trial flags and intersection primitive probe; excludes full solver rollback"}};
}

gipc::Json GIPC::line_search_rollback_self_test(device_TetraData& mesh)
{
    if(abd_fem_count_info.fem_point_num==0)
        throw std::runtime_error("rollback test requires FEM vertices");
    std::vector<double3> initial(vertexNum),saved_direction(vertexNum),restored(vertexNum);
    CUDA_SAFE_CALL(cudaMemcpy(initial.data(),mesh.vertexes,vertexNum*sizeof(double3),cudaMemcpyDeviceToHost));
    CUDA_SAFE_CALL(cudaMemcpy(saved_direction.data(),_moveDir,vertexNum*sizeof(double3),cudaMemcpyDeviceToHost));
    std::vector<gipc::Vector12> saved_q,saved_dq;
    m_abd_sim_data->device.body_id_to_q.copy_to(saved_q);
    m_abd_sim_data->device.body_id_to_dq.copy_to(saved_dq);
    auto& frame=gipc::Statistics::instance().at_current_frame();
    frame["newton"]=gipc::Json::array(); frame["newton"].push_back(gipc::Json::object());
    std::vector<double3> invalid_direction(vertexNum,make_double3(0,0,0));
    invalid_direction[abd_fem_count_info.fem_point_offset].x=std::numeric_limits<double>::infinity();
    CUDA_SAFE_CALL(cudaMemcpy(_moveDir,invalid_direction.data(),vertexNum*sizeof(double3),cudaMemcpyHostToDevice));
    std::vector<gipc::Vector12> dq(saved_q.size(),gipc::Vector12::Constant(0.01));
    m_abd_sim_data->device.body_id_to_dq.copy_from(dq);
    double alpha=1;
    bool rejected=false;
    try { lineSearch(mesh,alpha,1); }
    catch(const std::runtime_error& error)
    {
        rejected=std::string(error.what())=="line search failed: nonfinite_trial_positions";
    }
    CUDA_SAFE_CALL(cudaMemcpy(_moveDir,saved_direction.data(),vertexNum*sizeof(double3),cudaMemcpyHostToDevice));
    m_abd_sim_data->device.body_id_to_dq.copy_from(saved_dq);
    CUDA_SAFE_CALL(cudaMemcpy(restored.data(),mesh.vertexes,vertexNum*sizeof(double3),cudaMemcpyDeviceToHost));
    std::vector<gipc::Vector12> restored_q;
    m_abd_sim_data->device.body_id_to_q.copy_to(restored_q);
    bool positions_equal=true,q_equal=restored_q.size()==saved_q.size();
    for(std::size_t i=0;i<initial.size();++i)
        positions_equal &= restored[i].x==initial[i].x && restored[i].y==initial[i].y && restored[i].z==initial[i].z;
    for(std::size_t i=0;q_equal && i<saved_q.size();++i)
        q_equal &= (restored_q[i].array()==saved_q[i].array()).all();
    const auto& diagnostic=frame["newton"].back()["line_search"];
    if(!rejected || !positions_equal || !q_equal || diagnostic["status"]!="failed"
       || !diagnostic["accepted_alpha"].is_null())
        throw std::runtime_error("production line search did not reject and restore invalid trial");
    return {{"test","production_line_search_nonfinite_trial_rollback"},{"passed",true},
        {"vertices_restored_exactly",positions_equal},{"abd_q_restored_exactly",q_equal},
        {"abd_body_count",saved_q.size()},{"frames_committed",0},
        {"performance_claim",false},{"line_search",diagnostic}};
}
                      ...
                      double&           time4)
{
    ...
    for(; k < iterCap; ++k)
    {
        ...
        const auto galerkin_step = agipc::galerkin_step_summary();
...
int    totalNT          = 0;
double totalTime        = 0;
double total_statistics_flush_wall_ms=0;
int    total_Frames     = 0;
double ttime0           = 0;
double ttime1           = 0;
double ttime2           = 0;
...
void   GIPC::IPC_Solver(device_TetraData& TetMesh)
{
    ...
    if(isUpdateBoundary)
    {
        ...
        cudatool::CudaDeviceBuffer<double3> previous_boundary_trial;
        while(isIntersected(TetMesh))
        {
            const int flags=trial_flags(TetMesh.vertexes,
                previous_boundary_trial.size()?previous_boundary_trial.data():TetMesh.temp_double3Mem,vertexNum);
            if(numOfIntersect>=gipc::line_search_backtrack_limit || !(alpha*0.5>0)
               || (flags&2) || (numOfIntersect && !(flags&1)))
            {
                auto& failure=gipc::Statistics::instance().at_current_frame()["boundary_line_search"];
                failure={{"status","failed"},{"failure_reason","boundary_intersection_backtracking_failed"},
                         {"trial_alpha",alpha},{"intersection_backtracks",numOfIntersect}};
                CUDA_SAFE_CALL(cudaMemcpy(TetMesh.vertexes,TetMesh.temp_double3Mem,
                                          vertexNum*sizeof(double3),cudaMemcpyDeviceToDevice));
                buildBVH(); buildCP();
                failure["restored_initial_state"]=true;
                throw std::runtime_error("boundary intersection backtracking failed");
            }
            previous_boundary_trial.resize(vertexNum);
            CUDA_SAFE_CALL(cudaMemcpy(previous_boundary_trial.data(),TetMesh.vertexes,
                                      vertexNum*sizeof(double3),cudaMemcpyDeviceToDevice));
    ...
    const auto statistics_flush_begin=std::chrono::steady_clock::now();
    stats.write_to_file(std::string{gipc::output_dir()} + "/stats.json");
    total_statistics_flush_wall_ms+=std::chrono::duration<double,std::milli>(
        std::chrono::steady_clock::now()-statistics_flush_begin).count();
