...
struct CriterionTotals
{
    double low, high, sum;
    int invalid, protected_edges, strain_edges, boundary_edges;
};

struct ReadCriterionTotals
{
    const double* increments;
    const int* reasons;
    int element_count, edge_count;
    __host__ __device__ CriterionTotals operator()(int i) const
    {
        CriterionTotals t{INFINITY, 0, 0, 0, 0, 0, 0};
        if(i < element_count)
        {
            const double v = increments[i];
            if(!isfinite(v)) t.invalid = 1;
            else { t.low = v; t.high = v; t.sum = v; }
        }
        if(i < edge_count)
        {
            const int r = reasons[i];
            t.protected_edges = r != 0;
            t.strain_edges = (r & 1) != 0;
            t.boundary_edges = (r & 2) != 0;
        }
        return t;
    }
};

struct CombineCriterionTotals
{
    __host__ __device__ CriterionTotals operator()(CriterionTotals a, CriterionTotals b) const
    {
        return {a.low < b.low ? a.low : b.low, a.high > b.high ? a.high : b.high,
                a.sum + b.sum, a.invalid + b.invalid,
                a.protected_edges + b.protected_edges, a.strain_edges + b.strain_edges,
                a.boundary_edges + b.boundary_edges};
    }
};

CriterionTotals reduce_criterion_totals(const Workspace& w)
{
    const auto element_count=w.increments.size(), edge_count=w.reasons.size();
    return thrust::transform_reduce(thrust::device,
        thrust::make_counting_iterator(0),
        thrust::make_counting_iterator(static_cast<int>(std::max(element_count,edge_count))),
        ReadCriterionTotals{w.increments.data(),w.reasons.data(),
                            static_cast<int>(element_count),static_cast<int>(edge_count)},
        CriterionTotals{INFINITY,0,0,0,0,0,0},CombineCriterionTotals{});
}
...
int selected_affine_basis_mask(const std::vector<double3>& points,
                               bool paper_affine_basis)
{
    // Main paper Eq. (4) assigns one homogeneous coordinate and all three
    // rest-pose coordinates to every affine aggregate, i.e. 4 vector blocks
    // and 12 scalar DoF. Rank-aware trimming is retained only as an explicit
    // non-paper ablation for degenerate planar or linear aggregates.
    return paper_affine_basis ? 0xf : affine_basis_mask(points);
}
...
gipc::Json build_mapping(Workspace& w)
{
    ...
    for(int i=0;i<current;++i)
    {
        ...
        if(affine_flags[i])
        {
            basis_masks[i]=selected_affine_basis_mask(
                coarse_points[i],w.paper_affine_basis);
    ...
    return {{"method","paper_warp_hash"},{"fine_nodes",w.fine_vertices},
            {"affine_basis_mode",w.paper_affine_basis?"paper12":"rank-aware"},
                          ...
                          bool paper_affine_basis)
{
    ...
    w.paper_affine_basis = paper_affine_basis;
...
gipc::Json update_mapping()
{
    ...
    if(collect_timing_enabled()) CUDA_SAFE_CALL(cudaEventRecord(workspace.start));
    workspace.last_mapping=build_mapping(workspace);
    float elapsed=0;
    if(collect_timing_enabled())
    {
    ...
    workspace.last_mapping["mapping_ms"]=collect_timing_enabled()?gipc::Json(elapsed):gipc::Json(nullptr);
...
gipc::Json update_criterion(device_TetraData& mesh)
{
    ...
    if(collect_timing_enabled()) CUDA_SAFE_CALL(cudaEventRecord(w.start));
    launch_increment(w,mesh.vertexes,false);
    launch_tags(w,mesh.BoundaryType);
    const auto element_count=w.increments.size(), edge_count=w.reasons.size();
    const auto totals=reduce_criterion_totals(w);
    float elapsed=0;
    if(collect_timing_enabled())
    {
        CUDA_SAFE_CALL(cudaEventRecord(w.end));
        CUDA_SAFE_CALL(cudaEventSynchronize(w.end));
        CUDA_SAFE_CALL(cudaEventElapsedTime(&elapsed,w.start,w.end));
    }
    const int protected_count=totals.protected_edges, strain_count=totals.strain_edges,
              boundary_count=totals.boundary_edges, invalid_count=totals.invalid;
    const double low=totals.low, high=totals.high, sum=totals.sum;
    if(invalid_count) throw std::runtime_error("AGIPC criterion nonfinite element; Gate A failed");
    ++w.updates;
    w.protected_sum += protected_count;
    w.collapsible_sum += edge_count-protected_count;
    if(element_count)
    {
        w.increment_min=std::min(w.increment_min,low);
        w.increment_max=std::max(w.increment_max,high);
        w.increment_sum+=sum;
        w.increment_samples+=element_count;
    }
    gipc::Json result={{"stage","criterion_only"},{"fine_vertex_count",w.fine_vertices},
            {"edge_count",edge_count},{"protected_edge_count",protected_count},
            {"collapsible_edge_count",edge_count-protected_count},
            {"protected_edge_ratio",edge_count?double(protected_count)/edge_count:0.0},
            {"strain_protected_edges",strain_count},{"boundary_protected_edges",boundary_count},
            {"green_increment_min",element_count?low:0.0},
            {"green_increment_mean",element_count?sum/element_count:0.0},
            {"green_increment_max",high},{"threshold",w.threshold},
            {"criterion_ms",collect_timing_enabled()?gipc::Json(elapsed):gipc::Json(nullptr)},
...
gipc::Json criterion_summary()
{
    ...
    auto result=gipc::Json{{"stage","criterion_and_mapping_shadow"},{"newton_updates",w.updates},
            ...
            {"total_mapping_ms",collect_timing_enabled()?gipc::Json(w.mapping_total_ms):gipc::Json(nullptr)},
            {"history_convention","reset_at_subIP_initial_positions"}};
...
gipc::Json criterion_self_test()
{
    ...
    // An irregular domain exercises all reduction blocks and the unequal array tails.
    std::vector<double> reference_increments(4099);
    std::vector<int> reference_reasons(6151);
    CriterionTotals reference{INFINITY,0,0,0,0,0,0};
    for(std::size_t i=0;i<reference_increments.size();++i)
    {
        const double v=static_cast<double>(i%31)*0.001;
        reference_increments[i]=v;
        reference.low=std::min(reference.low,v); reference.high=std::max(reference.high,v);
        reference.sum+=v;
    }
    for(std::size_t i=0;i<reference_reasons.size();++i)
    {
        const int r=static_cast<int>(i%4); reference_reasons[i]=r;
        reference.protected_edges+=r!=0; reference.strain_edges+=(r&1)!=0;
        reference.boundary_edges+=(r&2)!=0;
    }
    w.increments.copy_from_host(reference_increments); w.reasons.copy_from_host(reference_reasons);
    const auto reduced=reduce_criterion_totals(w);
    require(reduced.low==reference.low && reduced.high==reference.high
            && std::abs(reduced.sum-reference.sum)<1e-10
            && reduced.invalid==0 && reduced.protected_edges==reference.protected_edges
            && reduced.strain_edges==reference.strain_edges
            && reduced.boundary_edges==reference.boundary_edges,
            "criterion GPU summary differs from independent CPU reference");
    reference_increments[1024]=std::numeric_limits<double>::infinity();
    reference_increments[4098]=std::numeric_limits<double>::quiet_NaN();
    w.increments.copy_from_host(reference_increments);
    require(reduce_criterion_totals(w).invalid==2,"criterion GPU summary must retain nonfinite gate");
    return {{"test","agipc_criterion"},{"passed",true},{"cases",cases+4},
...
gipc::Json mapping_self_test()
{
    ...
    ++cases;
    if(basis_width(selected_affine_basis_mask(planar,true))!=4
       || basis_width(selected_affine_basis_mask(linear,true))!=4)
        throw std::runtime_error("Gate D paper affine basis must keep all 12 DoF");
    ++cases;
    return {{"test","agipc_mapping"},{"passed",true},{"cases",cases},
            {"gpu_kernels",true},{"covered","indirect/protected/7-tail/32-vs-33-affine/hierarchy/stable-cross-group/determinism/paper12/rank-aware-ablation"}};
}
}
