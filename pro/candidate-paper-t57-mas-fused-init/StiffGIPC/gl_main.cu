...
#include <agipc/agipc_mas_apply.cuh>
#include <agipc/agipc_device_pcg_options.cuh>
...
void initScene()
{
    ...
    const bool agipc_core_enabled=runtime_options.solver==gipc::SolverMode::AGIPC
        || runtime_options.solver==gipc::SolverMode::AGIPCSymHessian;
    agipc::configure_instrumentation(runtime_options.collect_timing,
                                     runtime_options.agipc_diagnostics);
    if(runtime_options.agipc_diagnostics || agipc_core_enabled)
    {
        ...
        agipc::configure_galerkin(runtime_options.agipc_fine_correction_iterations,
                                  ...
                                  runtime_options.agipc_coarse_preconditioner == "mas32"
                                      || runtime_options.agipc_coarse_preconditioner == "mas32-factor",
                                  runtime_options.agipc_coarse_preconditioner == "mas32-factor",
        ...
        agipc::configure_direction_freeze(runtime_options.agipc_direction_freeze_path,
                                          ...
                                          runtime_options.agipc_direction_freeze_continue,
                                          runtime_options.agipc_direction_freeze_frames,
                                          runtime_options.agipc_direction_freeze_late_after_newton);
        agipc::initialize_criterion(tetMesh,runtime_options.agipc_threshold,
                                    runtime_options.agipc_max_levels,
                                    runtime_options.agipc_affine_basis == "paper12");
    }
...
extern double total_statistics_flush_wall_ms;
...
int run_headless()
{
    ...
    init_headless();
    if(!runtime_options.agipc_fine_replay_path.empty())
    {
        try {
            const auto result=ipc.replay_frozen_fine(runtime_options.agipc_fine_replay_path,runtime_options.metrics_path,d_tetMesh);
            std::cout<<result.dump(2)<<'\n';
            return result.value("passed",false)?0:3;
        } catch(const std::exception& e) { std::cerr<<"Fine replay failed: "<<e.what()<<'\n';return 3; }
    }
    if(runtime_options.line_search_rollback_self_test)
    {
        try
        {
            const auto result=ipc.line_search_rollback_self_test(d_tetMesh);
            if(!runtime_options.metrics_path.empty())
            {
                const std::filesystem::path path(runtime_options.metrics_path);
                if(path.has_parent_path()) std::filesystem::create_directories(path.parent_path());
                std::ofstream out(path); out<<result.dump(2);
                if(!out) throw std::runtime_error("could not write rollback test result");
            }
            std::cout<<result.dump(2)<<'\n';
            return 0;
        }
        ...
        {
            std::cerr<<"Rollback self-test failed: "<<error.what()<<'\n';
            return 3;
        }
    }
    ...
    gipc::Json contact_frames=gipc::Json::array();
    int contact_frame_count=0,first_contact_frame=0,last_contact_frame=0;
    ...
    {
        try { ipc.IPC_Solver(d_tetMesh); }
        catch(const std::exception& error)
        {
            gipc::Json failure={{"status","failed"},{"failure_reason",error.what()},
                {"frames_requested",runtime_options.frames},{"frames_completed",total_Frames},
                {"failed_frame",frame+1},{"collect_timing",runtime_options.collect_timing},
                {"contact_frames",contact_frames},{"contact_frame_count",contact_frame_count},
                {"first_contact_frame",first_contact_frame},{"last_contact_frame",last_contact_frame},
                {"agipc_mas_validation_backend",runtime_options.agipc_mas_validation_backend},
                {"agipc_mas_apply",runtime_options.agipc_mas_apply},
                {"agipc_coarse_pcg_batch",runtime_options.agipc_coarse_pcg_batch},
                {"agipc_step_statistics",runtime_options.agipc_step_statistics},
                {"statistics_flush_wall_time_ms",total_statistics_flush_wall_ms},
                {"statistics",gipc::Statistics::instance().json()}};
            if(!runtime_options.metrics_path.empty())
            {
                const std::filesystem::path path(runtime_options.metrics_path);
                if(path.has_parent_path()) std::filesystem::create_directories(path.parent_path());
                std::ofstream out(path); out<<failure.dump(2);
            }
        ...
        if(ipc.frozen_linear_diagnostics_complete())
            return 0;
        const bool active_contact=ipc.h_cpNum[0]>0 || ipc.h_gpNum>0;
        if(active_contact) {
            ++contact_frame_count;
            if(first_contact_frame==0) first_contact_frame=total_Frames;
            last_contact_frame=total_Frames;
        }
        contact_frames.push_back({{"frame",total_Frames},{"active_self_or_body_pairs",ipc.h_cpNum[0]},
            {"active_ground_pairs",ipc.h_gpNum},{"has_active_contact",active_contact}});
        if(capture_checkpoints
    ...
    const bool agipc_mode = runtime_options.solver == gipc::SolverMode::AGIPC
        || runtime_options.solver == gipc::SolverMode::AGIPCSymHessian;

    gipc::Json metrics;
    metrics["scene"]            = runtime_options.scene;
    metrics["solver"]           = runtime_options.solver == gipc::SolverMode::AGIPCSymHessian
                                      ? "agipc-symhessian"
                                      : runtime_options.solver == gipc::SolverMode::AGIPC
                                      ? "agipc-core" : "stiffgipc";
    ...
    if(agipc_active)
        metrics["agipc_affine_basis"] = runtime_options.agipc_affine_basis;
    if(agipc_active)
    ...
    metrics["agipc_mas_validation_backend"]=runtime_options.agipc_mas_validation_backend;
    metrics["agipc_mas_apply"]=runtime_options.agipc_mas_apply;
    metrics["agipc_coarse_pcg_batch"]=runtime_options.agipc_coarse_pcg_batch;
    metrics["agipc_guarded_fine_correction"]=runtime_options.agipc_guarded_fine_correction;
    metrics["agipc_guarded_fine_warm_start"]=runtime_options.agipc_guarded_fine_warm_start;
    metrics["agipc_step_statistics"]=runtime_options.agipc_step_statistics;
    metrics["statistics_flush_wall_time_ms"]=total_statistics_flush_wall_ms;
    metrics["contact_frames"]=contact_frames;
    metrics["contact_frame_count"]=contact_frame_count;
    metrics["direction_capture"]=agipc::direction_capture_summary();
    metrics["first_contact_frame"]=first_contact_frame;
    metrics["last_contact_frame"]=last_contact_frame;
    metrics["contact_scope"]="frame-end discrete barrier-active pairs; excludes broad-phase CCD candidates";
    metrics["agipc_mas_reuse"]=runtime_options.agipc_mas_reuse;
    metrics["collect_timing"]=runtime_options.collect_timing;
    metrics["agipc_tiny_dense_max_dofs"]=runtime_options.agipc_tiny_dense_max_dofs;
    metrics["collect_full_diagnostics"]=runtime_options.agipc_diagnostics;
...
int main(int argc, char** argv)
{
    ...
    if(runtime_options.solver == gipc::SolverMode::AGIPCPaper)
    {
        std::cerr << "Requested AGIPC paper solver unavailable: stackless paper BVH is not implemented.\n";
        return 2;
    }
    agipc::configure_coarse_pcg_batch(runtime_options.agipc_coarse_pcg_batch);
    agipc::configure_guarded_fine_correction(runtime_options.agipc_guarded_fine_correction);
    agipc::configure_guarded_fine_warm_start(runtime_options.agipc_guarded_fine_warm_start);
    agipc::configure_direct_mas_apply(runtime_options.agipc_mas_apply=="direct");
    agipc::configure_tiny_dense_limit(runtime_options.agipc_tiny_dense_max_dofs);
    gpu_mas32::configure_local_check_gemm(runtime_options.agipc_mas_validation_backend=="gemm");
    gpu_mas32::configure_local_check_tiled(runtime_options.agipc_mas_validation_backend=="tiled");
    agipc::configure_step_statistics_mode(runtime_options.agipc_step_statistics=="history"
        ?agipc::StepStatisticsMode::History:runtime_options.agipc_step_statistics=="compact"
        ?agipc::StepStatisticsMode::Compact:agipc::StepStatisticsMode::Automatic);
    ...
    if(runtime_options.agipc_self_test)
    {
        ...
        try
        {
            ...
            result["line_search_probe"] = agipc::line_search_probe_self_test();
            result["bvh_bounds"] = agipc::bvh_bounds_self_test();
            result["collision_pair_capacity"] = agipc::collision_pair_capacity_self_test();
            result["tiny_dense"] = agipc::tiny_dense_self_test();
            result["step_statistics"] = agipc::step_statistics_self_test();
            result["mas_apply"] = agipc::mas_apply_self_test();
            result["device_pcg"] = agipc::device_pcg_self_test();
            result["passed"] = result["criterion"]["passed"].get<bool>()
                               && result["mapping"]["passed"].get<bool>()
                               && result["galerkin"]["passed"].get<bool>()
                               && result["line_search_probe"]["passed"].get<bool>()
                               && result["bvh_bounds"]["passed"].get<bool>()
                               && result["collision_pair_capacity"]["passed"].get<bool>()
                               && result["tiny_dense"]["passed"].get<bool>()
                               && result["step_statistics"]["passed"].get<bool>()
                               && result["mas_apply"]["passed"].get<bool>()
                               && result["device_pcg"]["passed"].get<bool>();
            std::cout << result.dump(2) << '\n';
        }
        catch(const std::exception& error)
        {
