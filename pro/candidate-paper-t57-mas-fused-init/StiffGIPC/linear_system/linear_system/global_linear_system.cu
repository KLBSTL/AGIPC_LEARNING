...
void write_binary(const std::filesystem::path& path, const std::vector<T>& values)
{
    ...
    output.close();
    if(!output)throw std::runtime_error("binary diagnostic write failed: "+path.string());
}
}  // namespace

#include "fine_replay.inl"
#include "guarded_fine_correction.inl"
...
{
    last_build_timing={};
    m_fine_preconditioner_ready = false;
    ...
    const bool adoption_enabled=agipc::galerkin_adoption_enabled();
    const bool measure_stages=agipc::collect_timing_enabled();
    ...
    if(!adoption_enabled || frozen_diagnostics_pending)
    {
...
gipc::SizeT GlobalLinearSystem::solve_linear_system()
{
    ...
    if(!adopted && !m_fine_preconditioner_ready)
    {
        if(m_global_preconditioner)
            m_global_preconditioner->do_assemble(*gipc_global_triplet);
    ...
    if(adopted && agipc::guarded_fine_correction_enabled()) {
        const auto correction=guarded_fine_correction(agipc::galerkin_prolongated(m_x.size()),
            agipc::galerkin_prolongated_residual_squared());
        iter+=correction.value("iterations",0); // Includes rejected additional work.
        agipc::record_guarded_fine_correction(correction,m_x.data(),m_x.size());
    }
    ...
    if(measure_stages)
    {
        ...
        agipc::record_linear_solve_timing({
            ...
            {"guarded_fine_correction",adopted&&agipc::guarded_fine_correction_enabled()?static_cast<double>(solver_ms):0.0},
    ...
    if(agipc::collect_full_diagnostics_enabled() && !adopted)
        agipc::record_fallback_direction_quality(*gipc_global_triplet,
                                                 m_b.data(),m_x.data(),m_x.size());
    if(adopted) {
        const auto directory=agipc::freeze_accepted_direction(*gipc_global_triplet,m_b.data(),m_x.size());
        if(!directory.empty())for(auto& preconditioner:m_local_preconditioners)
            if(auto* abd=dynamic_cast<ABDPreconditioner*>(preconditioner.get()))
                write_binary(std::filesystem::path(directory)/"native_abd_inverse.f64x144.bin",abd->replay_inverse());
    }
    return iter;
}
