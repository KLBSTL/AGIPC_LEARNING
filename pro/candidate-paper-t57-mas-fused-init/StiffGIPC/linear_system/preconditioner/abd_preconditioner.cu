...
std::vector<Matrix12x12> ABDPreconditioner::replay_inverse() const
{
    std::vector<Matrix12x12> result;
    m_abd.abd_system_diag_preconditioner.copy_to(result);
    return result;
}

void ABDPreconditioner::replay_inverse(const std::vector<Matrix12x12>& inverse)
{
    if(inverse.size()!=m_sim_data.abd_fem_count_info().abd_body_num)
        throw std::runtime_error("ABD replay inverse dimension mismatch");
    for(const auto& value:inverse)
        if(!value.allFinite())throw std::runtime_error("ABD replay inverse is nonfinite");
    m_abd.abd_system_diag_preconditioner.copy_from(inverse);
}
