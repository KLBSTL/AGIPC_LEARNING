...
namespace {
__global__ void capacity_discrete_fixture(const double3* vertices, uint32_t* counts,
                                          int* indices, int4* pairs, int4* ccd);
__global__ void capacity_ccd_fixture(uint32_t* counts, int4* pairs);
}
...
namespace
{
__constant__ uint2 collision_pair_capacities={0xffffffffu,0xffffffffu};
__device__ int reserve_discrete_pair(uint32_t* counter)
{
    const uint32_t slot=atomicAdd(counter,1u);
    return slot<collision_pair_capacities.x && slot<collision_pair_capacities.y
        ? static_cast<int>(slot) : -1;
}
__device__ void append_ccd_pair(int4* pairs,uint32_t* counter,int4 pair)
{
    const uint32_t slot=atomicAdd(counter,1u);
    if(slot<collision_pair_capacities.y) pairs[slot]=pair;
}
}

void set_collision_pair_capacities(uint32_t discrete,uint32_t ccd)
{
    const uint2 value=make_uint2(discrete,ccd);
    CUDA_SAFE_CALL(cudaMemcpyToSymbol(collision_pair_capacities,&value,sizeof(value)));
}
    ...
    switch(dtype)
    {
        case 0: {
            ...
            if(d < dHat)
            {
                ...
                int cdp_idx=reserve_discrete_pair(_cpNum);
                if(cdp_idx<0) return false;
                                                    ...
                                                    int4* _ccd_collisionPair) noexcept
{
    ...
    append_ccd_pair(_ccd_collisionPair,_cpNum,make_int4(-id0 - 1, id1, id2, id3));
}
...
__global__ void _calcInternalAABB(const Node* _nodes, AABB* _bvs, uint32_t* flags, int number)
{
    ...
    {
        // Publish the completed child box and acquire the sibling box across SMs.
        // A relaxed atomicCAS plus writer-only threadfence does not establish
        // the reader's acquire ordering or visibility for the nonvolatile boxes.
        cuda::atomic_ref<uint32_t,cuda::thread_scope_device> ready(flags[parent]);
        const uint32_t old = ready.exchange(0,cuda::memory_order_acq_rel);
        if(old == 0xFFFFFFFF)
        {
            return;
        }
                ...
                if((_bodyID[idx] != _bodyID[_faces[obj_idx].x]) || (_bodyID[idx] == -1))
                {
                    ...
                    if(!(_btype[idx] >= 2 && _btype[_faces[obj_idx].x] >= 2
                        ...
                        if(idx != _faces[obj_idx].x && idx != _faces[obj_idx].y
                           && idx != _faces[obj_idx].z)
                        {
                            append_ccd_pair(_ccd_collisionPair,_cpNum,make_int4(-idx - 1,
                                          _faces[obj_idx].x,
                                          _faces[obj_idx].y,
                                          _faces[obj_idx].z));
                ...
                if(self_eid != obj_idx)
                {
                       ...
                       || (_bodyID[_edges[self_eid].x] == -1))
                    {
                        if(!(_btype[_edges[self_eid].x] >= 2
                            ...
                            {
                                append_ccd_pair(_ccd_collisionPair,_cpNum,make_int4(current_edge.x,
                                              current_edge.y,
                                              _edges[obj_idx].x,
                                              _edges[obj_idx].y));
                            }
...
// Historical publication sequence, used only by the independent bounds test.
__global__ void legacy_internal_bounds_diagnostic(const Node* nodes,AABB* boxes,uint32_t* flags,int count)
{
    ...
    {
        if(atomicCAS(flags+parent,0xffffffff,0)==0xffffffff) return;
        boxes[parent]=merge(boxes[nodes[parent].left_idx],boxes[nodes[parent].right_idx]);
        __threadfence();
...
__global__ void capacity_discrete_fixture(const double3* vertices, uint32_t* counts,
                                          int* indices, int4* pairs, int4* ccd)
{
    if(threadIdx.x<73) _checkPTintersection(vertices,0,1,2,3,1e-6,counts,indices,pairs,ccd);
}
__global__ void capacity_ccd_fixture(uint32_t* counts,int4* pairs)
{
    const int i=threadIdx.x;
    if(i<73) append_ccd_pair(pairs,counts,make_int4(i,i+1,i+2,i+3));
}
}

gipc::Json agipc::collision_pair_capacity_self_test()
{
    using cudatool::CudaDeviceBuffer;
    const int4 sentinel=make_int4(-123,-123,-123,-123);
    const auto equal=[](int4 a,int4 b) { return a.x==b.x && a.y==b.y && a.z==b.z && a.w==b.w; };
    CudaDeviceBuffer<double3> vertices(std::vector<double3>{make_double3(.25,.25,1e-6),
        make_double3(0,0,0),make_double3(1,0,0),make_double3(0,1,0)});
    CudaDeviceBuffer<uint32_t> counts(5);
    CudaDeviceBuffer<int> indices(std::vector<int>(5,-123));
    CudaDeviceBuffer<int4> discrete(std::vector<int4>(5,sentinel));
    CudaDeviceBuffer<int4> ccd(std::vector<int4>(7,sentinel));
    set_collision_pair_capacities(3,5);
    counts.reset_zero();
    capacity_discrete_fixture<<<1,128>>>(vertices.data(),counts.data(),indices.data(),discrete.data(),ccd.data());
    CUDA_SAFE_CALL(cudaGetLastError());
    std::vector<uint32_t> host_counts; std::vector<int4> pairs,ccd_pairs; std::vector<int> host_indices;
    counts.copy_to_host(host_counts); discrete.copy_to_host(pairs); ccd.copy_to_host(ccd_pairs); indices.copy_to_host(host_indices);
    bool discrete_ok=host_counts[0]==73;
    for(int i=0;i<3;++i) discrete_ok=discrete_ok && equal(pairs[i],make_int4(-1,1,2,3)) && equal(ccd_pairs[i],pairs[i]);
    for(int i=3;i<5;++i) discrete_ok=discrete_ok && equal(pairs[i],sentinel) && host_indices[i]==-123;
    for(int i=3;i<7;++i) discrete_ok=discrete_ok && equal(ccd_pairs[i],sentinel);
    ccd.copy_from_host(std::vector<int4>(7,sentinel)); counts.reset_zero();
    capacity_ccd_fixture<<<1,128>>>(counts.data(),ccd.data());
    CUDA_SAFE_CALL(cudaGetLastError()); counts.copy_to_host(host_counts); ccd.copy_to_host(ccd_pairs);
    bool ccd_ok=host_counts[0]==73 && equal(ccd_pairs[5],sentinel) && equal(ccd_pairs[6],sentinel);
    for(int i=0;i<5;++i) ccd_ok=ccd_ok && ccd_pairs[i].x>=0 && ccd_pairs[i].x<73
        && equal(ccd_pairs[i],make_int4(ccd_pairs[i].x,ccd_pairs[i].x+1,ccd_pairs[i].x+2,ccd_pairs[i].x+3));
    ccd.copy_from_host(std::vector<int4>(75,sentinel)); counts.reset_zero();
    set_collision_pair_capacities(3,73);
    capacity_ccd_fixture<<<1,128>>>(counts.data(),ccd.data());
    CUDA_SAFE_CALL(cudaGetLastError()); counts.copy_to_host(host_counts); ccd.copy_to_host(ccd_pairs);
    std::vector<int> ids;
    bool retry_ok=host_counts[0]==73 && equal(ccd_pairs[73],sentinel) && equal(ccd_pairs[74],sentinel);
    for(int i=0;i<73;++i) { ids.push_back(ccd_pairs[i].x); retry_ok=retry_ok && equal(ccd_pairs[i],make_int4(ccd_pairs[i].x,ccd_pairs[i].x+1,ccd_pairs[i].x+2,ccd_pairs[i].x+3)); }
    std::sort(ids.begin(),ids.end());
    for(int i=0;i<73;++i) retry_ok=retry_ok && ids[i]==i;
    set_collision_pair_capacities(0xffffffffu,0xffffffffu);
    const bool passed=discrete_ok && ccd_ok && retry_ok;
    if(!passed) throw std::runtime_error("collision pair capacity sentinel/rebuild test failed");
    return {{"passed",passed},{"required_pairs",73},{"discrete_capacity",3},{"ccd_capacity",5},
        {"discrete_sentinels_intact",discrete_ok},{"ccd_sentinels_intact",ccd_ok},{"expanded_rebuild_complete",retry_ok}};
}

gipc::Json agipc::bvh_bounds_self_test()
{
    using cudatool::CudaDeviceBuffer;
    std::size_t fixed_failures=0,legacy_failures=0,checked_internal_nodes=0;
    for(int leaves:{1031,65537})
    {
        const int node_count=2*leaves-1;
        std::vector<Node> tree(node_count);
        for(int i=0;i<node_count;++i)
            tree[i]={i==0?0xffffffffu:static_cast<uint32_t>((i-1)/2),
                i<leaves-1?static_cast<uint32_t>(2*i+1):0xffffffffu,
                i<leaves-1?static_cast<uint32_t>(2*i+2):0xffffffffu,
                i<leaves-1?0xffffffffu:static_cast<uint32_t>(i-leaves+1)};
        CudaDeviceBuffer<Node> nodes(tree);
        CudaDeviceBuffer<AABB> bounds;
        CudaDeviceBuffer<uint32_t> flags(leaves-1);
        for(int frame=0;frame<8;++frame)
        {
            std::vector<AABB> input(node_count),oracle(node_count);
            for(int i=leaves-1;i<node_count;++i)
            {
                const double x=(i%71)-35.0+frame*7;
                const double y=(i%53)-26.0-frame*3;
                const double z=(i%37)-18.0+frame*2;
                input[i].lower=make_double3(x,y,z);
                input[i].upper=make_double3(x+1,y+2,z+3);
            }
            oracle=input;
            for(int i=leaves-2;i>=0;--i)
            {
                const auto& a=oracle[2*i+1]; const auto& b=oracle[2*i+2];
                oracle[i].lower=make_double3(std::min(a.lower.x,b.lower.x),std::min(a.lower.y,b.lower.y),std::min(a.lower.z,b.lower.z));
                oracle[i].upper=make_double3(std::max(a.upper.x,b.upper.x),std::max(a.upper.y,b.upper.y),std::max(a.upper.z,b.upper.z));
            }
            for(bool legacy:{true,false})
            {
                bounds.copy_from_host(input);
                CUDA_SAFE_CALL(cudaMemset(flags.data(),0xff,(leaves-1)*sizeof(uint32_t)));
                if(legacy)
                    legacy_internal_bounds_diagnostic<<<(leaves+255)/256,256>>>(nodes.data(),bounds.data(),flags.data(),leaves);
                else calcInternalAABB(nodes.data(),bounds.data(),flags.data(),leaves);
                CUDA_SAFE_CALL(cudaGetLastError());
                std::vector<AABB> actual; bounds.copy_to_host(actual);
                std::size_t failures=0;
                for(int i=0;i<leaves-1;++i)
                {
                    const auto a=actual[i],b=oracle[i];
                    failures+=a.lower.x!=b.lower.x || a.lower.y!=b.lower.y || a.lower.z!=b.lower.z
                        || a.upper.x!=b.upper.x || a.upper.y!=b.upper.y || a.upper.z!=b.upper.z;
                }
                if(legacy) legacy_failures+=failures;
                else { fixed_failures+=failures; checked_internal_nodes+=leaves-1; }
            }
        }
    }
    if(fixed_failures) throw std::runtime_error("BVH published bounds differ from independent CPU oracle");
    return {{"passed",true},{"checked_internal_nodes",checked_internal_nodes},
        {"fixed_bound_mismatches",fixed_failures},{"legacy_bound_mismatches",legacy_failures},
        {"scope","two irregular balanced tree sizes, eight changing geometries each; production internal AABB kernel"}};
}
