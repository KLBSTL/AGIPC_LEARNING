...
- [x] Move the same finite/SPD/inverse-residual checks to GPU with CPU/crosscheck modes. Real1415/741 snapshots agree; six artificial accept/reject cases pass. The three27-frame CPU/GPU/reuse-crosscheck ablations pass with no local failure/fallback and at most0.06470% RMS state difference relative to the fresh CPU control.
- [x] Add explicit exact-BCOO-structure hierarchy/workspace reuse, refreshing all values and local inverses each solve. The crosscheck27 run verifies117/117 adoptions and57 real reuse calls. GPU adjacency remains open; occupied-GPU timings are diagnostic only.
- [x] Complete a same-build35-frame Jacobi/MAS-reuse contact pair: finite, zero ground penetration,589/678 versus639/640 adoptions. Preserve the negative quality result:2.46746% relative RMS terminal difference, one MAS local rejection,78 Jacobi coarse-cap fallbacks, and8.44% MAS reuse. This is not a numerical-equivalence or speed gate pass.
- [x] Prepare bounded local-rejection CPU/GPU records and an explicit diagnostic failure-freeze category; final Release build/core self-test pass. Actual triggering/replay under the new instrumentation remains unvalidated.
- [x] Freeze one accepted full fine H/g at update300/frame31 and independently verify fine/coarse SparseLU references, real mixed/FEM Galerkin assembly and CPU10/20 post-correction. Reference gate passes; GPU10 has98.33% direction error and9.50% predicted decrease versus direct. Six affine groups cover98.35% of FEM nodes,96.49% of candidate squared error; even the best Euclidean subspace fit has93.87% error. This is a negative direction-quality result, not a long-contact numerical/performance pass. See `AGIPC_ACCEPTED_DIRECTION_QUALITY.md`.
- [x] Trace the giant affine groups with one explicit criterion snapshot. Independent CPU replay reconstructs current Green to `8.20e-15` relative error and all 49,408 edge tags exactly. At threshold `5e-5`, the allowed graph has a 14,379-node global component; runtime warp-hash remains finer at 2,202 groups and has a substantially better best direction fit than forced global merging. The paper leaves adaptive-label fixed-point continuation unspecified; preserve default post10/threshold and current stable mapping. See `AGIPC_CRITERION_SNAPSHOT_ANALYSIS.md`.
- [x] Add explicit low-volume full-FEM frame checkpoints and compare one same-build35-frame Jacobi/MAS pair. Frames25/26 agree within `4.7e-9` RMS; the first material state split is frame27. Newton2/update56 first differs by one protected edge and two coarse nodes, then mapping/line-search differences amplify. See `AGIPC_FRAME_CHECKPOINT_DIVERGENCE.md`.
- [ ] Freeze one update55/56 system and replay Jacobi/MAS coarse solve, prolongation and post10 on the identical H/g and mapping. Measure threshold margin for the first tag flip; do not repeat the35-frame pair.
- [ ] Locate nonlinear state divergence with sparse checkpoints and independently verify a captured local rejection. The frozen frame31 directions all exceed the FEM Newton threshold, so this sample does not demonstrate premature stopping.
...
- [x] Run one user-requested preliminary pair after one-frame warmups. The configuration gate passes, but pre-run GPU medians24%/36%, fixed order, one repetition and3.5029% terminal RMS state difference reject a formal claim. Observed AGIPC/Stiff time ratio is1.2207; coarse solve is56.16% of AGIPC time. See `AGIPC_RECENT_EXPERIMENT_SYNTHESIS_2026-09-15.md`.
