...
The next Release build exports complete original-block replay vectors. Both real inputs exit0; all four vectors independently pass size, finite-value, CPU true-residual and GPU/CPU residual-consistency checks. Direct-reference Euclidean errors are 4.4101%/4.1474% (Jacobi/MAS, 1415 blocks) and 4.2962%/1.1550% (741 blocks). All padding vector norms are zero. A copied large input with max_iterations=1 exits3, overall false, and both methods execute exactly one iteration with `iteration_cap`; this is an artificial gate, not a captured real failure. Final `--agipc-self-test` exits0 with overall true. Build, commands, hash and timing boundaries are recorded in `AGIPC_COARSE_VECTOR_AND_COST_ANALYSIS.md`.

## Runtime coarse MAS32 experiment

The explicit `--agipc-coarse-preconditioner mas32` route and default Jacobi both exit0 on the same-build 27-frame16K cloth case without capture diagnostics. Both are finite, ground-penetration-free and keep the ABD fixed below5.56e-17. MAS validates and adopts83/83 directions with no local failure/fallback,56 applied Newton updates and2367 total linear iterations; Jacobi adopts77/85 with8 coarse-invalid-preconditioned-residual fallbacks,58 updates and6150 iterations. The paired16,641-vertex RMS state error is0.0001946583 (0.05260% of Jacobi RMS displacement); MAS and Jacobi are within0.127%/0.128% of the historical matched-stop fine baseline. Maximum pointwise errors are separately retained in the report.

The final Release build and core self-test pass. Invalid coarse names and using coarse MAS in StiffGIPC mode exit2. MAS local checks and exact coarse-residual checks run before fine post-correction/adoption; default Jacobi semantics remain unchanged. Cost and integrity limits, exact commands and SHA256 are in `AGIPC_RUNTIME_COARSE_MAS32.md`. Full CPU local diagnostics dominate this initial runtime; no controlled speedup is claimed.

## GPU local checks and topology reuse (2026-09-13)

The new Release build and MAS hierarchy/local self-test pass. Six artificial cases correctly accept identity and reject local/inverse indefiniteness, local/inverse NaN, and an SPD inverse with excessive residual. Real1415/741 frozen systems pass replay and CPU/GPU diagnostic agreement; maximum inverse residuals are5.15083e-5/2.14053e-5.

Matched27-frame CPU, GPU, and crosscheck+reuse cases all exit0 with finite vertices, zero ground penetration, fixed ABD displacement5.55e-17 and maximum139 self-contact pairs. They adopt94/94,89/89,117/117 candidates with no local rejection/fallback. Crosscheck verifies57 exact-structure reuse calls while always refreshing coefficients/inverses. Relative RMS terminal differences to the fresh CPU case are0.06121% and0.06470%; maximum pointwise differences0.002013/0.002225 are separately retained. Different update trajectories exclude bitwise equivalence and controlled timing claims. Detailed evidence and limitations are in `AGIPC_MAS_BATCH_OPTIMIZATION.md`.

The35-frame default-Jacobi/MAS-reuse pair also exits0, remains finite/ground-penetration-free and keeps the ABD fixed. Jacobi has643 applied updates,107570 linear iterations and89 fallbacks (78 coarse caps/11 invalid preconditioned residuals); MAS has605 updates,98750 iterations and1 local diagnostic rejection. MAS adopts639/640 versus Jacobi589/678, but terminal RMS difference0.0143181991 is2.46746% of Jacobi displacement, maximum0.0726423534, with216/221 maximum self-contact pairs. Higher adoption does not establish long-contact state equivalence. No intermediate state checkpoints or rejected matrix were saved; same-H/g direction comparisons and failure replay are the next numerical priorities.

Bounded paired rejection records and a diagnostic `local_diagnostics_failure` freeze category are now prepared. Their final Release build and one comprehensive `--agipc-self-test` exit0/pass; this verifies core protection, not actual triggering of the new failure category. Scene data uses archived FE0A... binary, while instrumentation/core validation uses A7FF...; hashes and exact boundaries are retained in the batch report.

## Criterion snapshot replay, 2026-09-15

An accepted update300 system now includes an explicit criterion snapshot. Independent CPU reconstruction agrees with all saved current Green tensors to `8.20e-15` relative error and reproduces all 49,408 edge reason/tag values exactly. The captured default threshold protects 12,485 edges (25.2692%). Code and paper-timing audit confirms one reset at subIP entry followed by current-versus-previous-Newton updates; the signed previous Green tensor was not exported, so the delta norm history is not independently reconstructed.

The allowed-edge graph has 1,958 global connected components, including one 14,379-node component. Runtime recursive warp hashing stops at a finer stable partition of 2,202 groups/2,294 mixed blocks, leaving 2,193 allowed cross-group edges. It never merges distinct allowed components. Its best Euclidean fit to the saved fine direct direction has 62.8589% relative error, versus 84.9642% for forced global component merging. The actual GPU10 direction has 87.5464% Euclidean and 73.2104% H-energy error, retaining 46.4023% of the direct quadratic decrease. The independent direct/Galerkin/replay gates pass.

An offline same-increment threshold scan shows a sharp connectivity transition between `2.5e-5` and `5e-5`: global-component active blocks fall from 48.98% to 11.78%, while best Euclidean fit error rises from 44.45% to 84.96%. This is a one-system diagnostic and does not justify changing the paper threshold. Details, exact commands and boundaries are in `AGIPC_CRITERION_SNAPSHOT_ANALYSIS.md`.

## Same-build frame checkpoint divergence, 2026-09-15

Explicit full-FEM FP64 checkpoints for the initial state and frames25--35 pass structural and finite-value checks. A same-executable35-frame block-Jacobi/MAS32+reuse pair completes with finite vertices and zero ground penetration. Frame25/26 RMS differences remain below`4.7e-9`; frame27 rises to`2.419e-4`, frame28 to`1.846e-3`, and frame35 to`0.0123796` or2.13368% of Jacobi RMS displacement. Maximum frame35 pointwise difference is`0.0473585`.

Aligned Newton records locate the first edge/mapping mismatch at frame27 Newton2/update56: protected edges4312/4313 and coarse nodes1317/1319. The previous Newton has zero protected edges, one coarse group and only`1.247e-4` relative direction-norm difference. One Newton later, protected counts are3434/4356 and accepted alphas0.85609/1.0. The first applied adoption outcome mismatch is later at frame28 Newton9, so adoption gating is not the initial branch. This supports threshold amplification of small solver differences without establishing a correctness bug. See `AGIPC_FRAME_CHECKPOINT_DIVERGENCE.md`.

The subsequent requested one-pair performance run uses no checkpoints or freezes. StiffGIPC and AGIPC-Core both finish35 frames with finite vertices and zero ground penetration, but their16,641-node terminal RMS difference is`0.0203309` (3.50287% of StiffGIPC RMS displacement), maximum`0.0768556`. AGIPC adopts555/557 candidates and has only two fallbacks, yet the state-equivalence gate remains failed. The observed50.1772s/41.1049s timing is excluded from formal performance claims because GPU load differs and only one fixed-order pair was measured. Full synthesis: `AGIPC_RECENT_EXPERIMENT_SYNTHESIS_2026-09-15.md`.
